...
const dialogTitle = computed(() => {
  return props.mode === 'add' ? 'Add New Point Rule' : 'Edit Point Rule'
})
...
const initializeForm = () => {
  if (props.mode === 'edit' && props.item) {
    formData.value = {
      ...
      points_value: props.item.points_value !== undefined ? Number(props.item.points_value) : 0,
...
const handleSubmit = async () => {
  ...
  if (props.mode === 'add') {
    // POST - Create
    ApiService.post('/points', payload,
      (data) => {
        emit('submit')
        closeDialog()
        loading.value = false
    ...
    // PATCH - Update
    ApiService.patch(`/points/${encryptIt(String(props.item.id))}`, payload,
      (data) => {
        emit('submit')
        closeDialog()
        loading.value = false
          ...
          <VRow>
            ...
            <VCol cols="12">
              <AppTextField
                ...
                label="Action Type"
                placeholder="e.g., daily_login, task_complete, achievement"
                :rules="[requiredValidator]"
                :error-messages="errors.action_type"
                required
              />
            </VCol>
            ...
            <VCol cols="12">
              <AppTextField
                v-model="formData.points_value"
                label="Points Value"
                type="number"
                placeholder="Enter points value (positive or negative)"
                :rules="[requiredValidator]"
                :error-messages="errors.points_value"
                required
              />
            </VCol>
