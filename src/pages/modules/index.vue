...
const confirmDelete = async () => {
  ...
  loading.value = true
  ApiService.delete(`/modules/${selectedModule.value.id}`,
    () => {
  ...
  <div>
    ...
    <div class="mb-6">
      <div class="d-flex align-center justify-space-between mb-2">
        <div>
          <h2 class="text-h4 mb-1">Modules</h2>
          <p class="text-body-1 text-medium-emphasis mb-0">
            Manage module records
          </p>
        </div>
        <VBtn v-if="can('MODULES.CREATE')" color="primary" prepend-icon="tabler-plus" @click="handleAdd">
          Add Module
        </VBtn>
      </div>
    </div>
      ...
      <VDataTable
        ...
        <template #item.actions="{ item }">
          <VMenu>
            ...
            <VList>
              <VListItem v-if="can('MODULES.UPDATE')" @click="handleEdit(item)">
                <template #prepend>
                  <VIcon icon="tabler-edit" size="20" />
                </template>
                <VListItemTitle>Edit</VListItemTitle>
              </VListItem>
              <VListItem v-if="can('MODULES.DELETE')" @click="handleDelete(item)">
                <template #prepend>
        ...
        <template #no-data>
          <div class="text-center py-8">
            ...
            <p class="text-body-1 text-medium-emphasis">No modules found</p>
          </div>
        </template>
      </VDataTable>
